...
def get_shapes(shapes,shape_path):
    shape_series = []
    for shape in shapes:
        shape_series.append(pd.read_csv(shape_path + '/' + 'Wt' + '_' + shape + '.csv'))
...
Wt_shape=get_shapes(shapes=['EP','HeIT','MGW','ProT','Roll'],shape_path=r'D:\DNA_shape')

#read hand-crafted biological feature
bio_feature = pd.read_csv(r'D:\Wt_biofeature.csv',header=None)
bio_feature = np.array(bio_feature)
#read secondary structure feature
x_data_RNA_Struct = pd.read_csv(r'D:\Wt_RNAencoding.csv',header=None)
...
x_train_posi,x_test_posi,x_train_bio,x_test_bio,x_train_oneRNA,x_test_oneRNA,x_train_one,x_test_one, x_train_shape,x_test_shape,y_train, y_test = train_test_split(x_data2,bio_feature,model_input1,x_data1, Wt_shape,y_data, test_size=0.15,random_state=1)

x_train_one=x_train_one.reshape(47263,1*21,4)
x_test_one=x_test_one.reshape(8341,1*21,4)

x_train_oneRNA=x_train_oneRNA.reshape(47263,1*21,5)
x_test_oneRNA=x_test_oneRNA.reshape(8341,1*21,5)

x_train_posi=x_train_posi.reshape(47263,1*21*1)
x_test_posi=x_test_posi.reshape(8341,1*21*1)
...
def cnn_block2(input):
    data_Conv1 = Convolution1D(filters=30, kernel_size=1, padding='same', activation='relu')(input)
    data_Conv3 = Convolution1D(filters=30, kernel_size=3, padding='same', activation='relu')(data_Conv1)
...
def DeepMEns_WT(shape1,shape2,shape3,shape4):
    ...
    for train_index, valid_index in kf.split(x_train_one):
        ...
        model.save(r'D:\Wt_DeepMEns_{0}.h5'.format(N))
        model.summary()
        end = time.time()
        print('infer_time:', end-start)
        N = N + 1
#f=DeepMEns_WT(shape1=(21,5),shape2=(21,5),shape3=(21,),shape4=(17,))

def ensmble_predict_shape5_oneRNA5_posi(inputs):
    model0 = load_model(r'D:\Wt_DeepMEns_0.h5',custom_objects={'TransformerEncoder': TransformerEncoder})
    model1 = load_model(r'D:\Wt_DeepMEns_1.h5',custom_objects={'TransformerEncoder': TransformerEncoder})
    model2 = load_model(r'D:\Wt_DeepMEns_2.h5',custom_objects={'TransformerEncoder': TransformerEncoder})
    model3 = load_model(r'D:\Wt_DeepMEns_3.h5',custom_objects={'TransformerEncoder': TransformerEncoder})
    model4 = load_model(r'D:\Wt_DeepMEns_4.h5',custom_objects={'TransformerEncoder': TransformerEncoder})
    y_pred0 = model0.predict(inputs)
    y_pred1 = model1.predict(inputs)
    y_pred2 = model2.predict(inputs)
    y_pred3 = model3.predict(inputs)
    y_pred4 = model4.predict(inputs)
    y_pred =  (y_pred0+y_pred1+y_pred2+y_pred3+y_pred4)/5
    ...
    spermanrs = sp.stats.spearmanr(y_test, y_pred)[0]
    mse = np.mean(np.square(np.array(y_pred) - np.array(y_test)))
    y_test1 = y_test.reshape(-1, )
    y_test_pred = y_pred.reshape(-1, )
    pearson = sp.stats.pearsonr(y_test1, y_test_pred)[0]

    spermanrs0 = sp.stats.spearmanr(y_test, y_pred0)[0]
    spermanrs1 = sp.stats.spearmanr(y_test, y_pred1)[0]
    spermanrs2 = sp.stats.spearmanr(y_test, y_pred2)[0]
    spermanrs3 = sp.stats.spearmanr(y_test, y_pred3)[0]
    spermanrs4 = sp.stats.spearmanr(y_test, y_pred4)[0]

    y_test_pred0 = y_pred0.reshape(-1, )
    pearson0 = sp.stats.pearsonr(y_test1, y_test_pred0)[0]
    y_test_pred1 = y_pred1.reshape(-1, )
    pearson1 = sp.stats.pearsonr(y_test1, y_test_pred1)[0]
    y_test_pred2 = y_pred2.reshape(-1, )
    pearson2 = sp.stats.pearsonr(y_test1, y_test_pred2)[0]
    y_test_pred3 = y_pred3.reshape(-1, )
    pearson3 = sp.stats.pearsonr(y_test1, y_test_pred3)[0]
    y_test_pred4 = y_pred4.reshape(-1, )
    pearson4 = sp.stats.pearsonr(y_test1, y_test_pred4)[0]

    mse0 = np.mean(np.square(np.array(y_pred0) - np.array(y_test)))
    mse1 = np.mean(np.square(np.array(y_pred1) - np.array(y_test)))
    mse2 = np.mean(np.square(np.array(y_pred2) - np.array(y_test)))
    mse3 = np.mean(np.square(np.array(y_pred3) - np.array(y_test)))
    mse4 = np.mean(np.square(np.array(y_pred4) - np.array(y_test)))
    ...
    print('spermanrs', spermanrs)
    print('pearson', pearson)
    print('mse', mse)
    return y_pred
g=ensmble_predict_shape5_oneRNA5_posi([x_test_shape,x_test_oneRNA,x_test_posi,x_test_bio])
