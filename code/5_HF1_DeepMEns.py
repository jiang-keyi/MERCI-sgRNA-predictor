...
FILE = pd.read_csv(r'D:\SpCas9HF1.csv')
...
def get_shapes(shapes,shape_path):
    shape_series = []
    for shape in shapes:
        shape_series.append(pd.read_csv(shape_path + '/' + 'HF1' + '_' + shape + '.csv'))
...
bio_feature = pd.read_csv(r'D:\HF_biofeature.csv',header=None)
bio_feature = np.array(bio_feature)
#read secondary structure feature
x_data_RNA_Struct = pd.read_csv(r'D:\SpCas9HF1_RNAencoding.csv',header=None)
...
x_train_posi,x_test_posi,x_train_bio,x_test_bio,x_train_oneRNA,x_test_oneRNA,x_train_one,x_test_one, x_train_shape,x_test_shape,y_train, y_test = train_test_split(x_data2,bio_feature,model_input1,x_data1,f,y_data, test_size=0.15,random_state=1)

x_train_one=x_train_one.reshape(48354,1*21,4)
x_test_one=x_test_one.reshape(8534,1*21,4)

x_train_shape=x_train_shape.reshape(48354,1*21,5)
x_test_shape=x_test_shape.reshape(8534,1*21,5)

x_train_oneRNA=x_train_oneRNA.reshape(48354,1*21,5)
x_test_oneRNA=x_test_oneRNA.reshape(8534,1*21,5)

x_train_posi=x_train_posi.reshape(48354,1*21*1)
x_test_posi=x_test_posi.reshape(8534,1*21*1)



def cnn_block1(input):
    data_Conv1 = Convolution1D(filters=40, kernel_size=1, padding='same', activation='relu')(input)
    data_Conv2 = Convolution1D(filters=40, kernel_size=2, padding='same', activation='relu')(input)
    data_Conv3 = Convolution1D(filters=40, kernel_size=3, padding='same', activation='relu')(input)
    data_Conv4 = Convolution1D(filters=40, kernel_size=4, padding='same', activation='relu')(input)
    data_Conv5 = Convolution1D(filters=40, kernel_size=5, padding='same', activation='relu')(input)
...
def cnn_block2(input):
    data_Conv1 = Convolution1D(filters=40, kernel_size=1, padding='same', activation='relu')(input)
    data_Conv3 = Convolution1D(filters=40, kernel_size=3, padding='same', activation='relu')(data_Conv1)
    data_Conv5 = Convolution1D(filters=40, kernel_size=5, padding='same', activation='relu')(data_Conv3)
...
def DeepMEns_HF1(shape1, shape2, shape3, shape4):
    ...
    for train_index, valid_index in kf.split(x_train_one):
        ...
        model.save(r'D:\HF1_DeepMEns_{0}.h5'.format(N))
        model.summary()
        N = N + 1
